...
def test(args, online_model, writer, T=None):
    ...
    writer.add_scalar('Average Rewards', sum(avg_rewards) / args.evaluation_episodes, T)
    writer.add_scalar('Average Episode Lengths', sum(avg_episode_lengths) / args.evaluation_episodes, T)
    
    torch.save(online_model.state_dict(), 'model.pth')  # Save model params
